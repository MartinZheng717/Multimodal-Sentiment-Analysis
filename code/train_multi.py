...
parser.add_argument('--max_len', type=int, default=32, help='the maximun length of the sequence')
...
def train_and_eval(model, train_dataloader, valid_dataloader, optimizer,scheduler):
    ...
    for i in range(args.epoch):
        ...
        if acc > best_acc:
            best_acc = acc
            best_weights_path = os.path.join(args.best_weights_path, "best_model.pth")
...
def main():
    ...
    args.log = os.path.join(args.log, 'Train-Multi{}'.format(time.strftime("%H-%M-%S")))
    ...
    # load data
    print("Loading data")
    args.pretrained_text_model_path = "code/model/pretrained/sentiment-roberta"
    data_loader=MultimodalDataloader(args.training_set_path,
                                     args.image_path,
                                     args.text_path,
