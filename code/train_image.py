...
import numpy as np
import pandas as pd
from tool.dataloader import ImageDataloader
...
def main():
    ...
    args.log = os.path.join(args.log, 'Train-Image-{}'.format(time.strftime("%H-%M-%S")))
    ...
    # load data
    print("Loading data")
    data_loader=ImageDataloader(args.training_set_path,
                                args.image_path,
                                transform,
    ...
    model = ImageModel(pretrained_ImageModel=args.pretrained_image_model_path)
    # 优化器
